...
class GaussianModel:
    ...
    def LOD_residual(self, points_sample_interval):
        ch_num = self.args.ch_num
        basis_num = self.args.curve_num
        N = len(self._xyz)

        depths = points_sample_interval.view(N ,1 ,1 ,1)
    ...
    def LOD_control(self, points_sample_interval):
        LOD_res = self.LOD_residual(points_sample_interval)
        opacity_final = self.opacity_activation(self._opacity + LOD_res[:,:1])
        scales_final = self.scaling_activation(self._scaling + LOD_res[:,1:4])
        color_residual = LOD_res[:,4:]
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float, focal_x: float):
        ...
        position_coefs = torch.zeros((N, self.args.ch_num, self.args.curve_num)) + torch.linspace(0, 15.0/focal_x, self.args.curve_num) #@Zhenya: 15.0/focal_x is for blender
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._coefs], 'lr': training_args.gmm_lr_init * self.spatial_lr_scale, "name": "coefs"}
        ]
        ...
        self.gmm_scheduler_args = get_expon_lr_func(lr_init=training_args.gmm_lr_init*self.spatial_lr_scale,
                                                    lr_final=training_args.gmm_lr_final*self.spatial_lr_scale,
                                                    lr_delay_mult=training_args.gmm_lr_delay_mult,
                                                    max_steps=training_args.position_lr_max_steps)
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                ...
                param_group['lr'] = lr
            if param_group["name"] == "coefs":
                lr = self.gmm_scheduler_args(iteration)
                param_group['lr'] = lr
